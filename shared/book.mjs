...
export function validateBook(data) {
  ...
  book.pages=data.pages.map(p=>({id:uid(),title:cleanText(p.title,160),text:cleanText(p.text,3000),scene:cleanText(p.scene,3000),sourceNote:cleanText(p.sourceNote,1000),image:validImage(p.image)?p.image:'',alt:cleanText(p.alt,1000)}));
  return book;
}
